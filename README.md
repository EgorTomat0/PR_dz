# 🚀 Стажировка Авито, сервис назначения ревьюеров для Pull Request'ов

## 📋 Оглавление

- [🚀 Стажировка Авито, сервис назначения ревьюеров для Pull Request'ов](#-стажировка-авито-сервис-назначения-ревьюеров-для-pull-requestов)
    - [📋 Оглавление](#-оглавление)
    - [🎯 Quick Start](#-quick-start)
        - [Установка и запуск](#установка-и-запуск)
    - [🏗 Архитектура проекта](#-архитектура-проекта)
        - [Описание ключевых компонентов](#описание-ключевых-компонентов)
    - [⚡ Нагрузочное тестирование при помощи K6](#-нагрузочное-тестирование-при-помощи-k6)
        - [Конфигурация теста](#конфигурация-теста)
        - [Результаты тестирования](#результаты-тестирования)

## 🎯 Quick start

### Установка и запуск

```bash
git clone https://github.com/EgorTomat0/avito_internship_PR

cd avito_internship_PR
```
```bash
docker compose up
```
Или (если уже установлен task)
```bash
task up:docker-compose
```

После выполнения этих команд приложение будет доступно по адресу: http://localhost:8080

### Локальное использование
Для этого можно применять Taskfile
Гайд по установке: https://taskfile.dev/docs/installation#official-package-managers
````bash
#Для запуска линтера из корневой директории
task lint
````
````bash
#Для запуска самого проекта
task up:local
````
````bash
#Для запуска с помощью docker compose
task up:docker-compose
````

## 🏗 Архитектура проекта

```markdown
avito_internship_PR/
├── cmd/
│   └── main.go                 # Точка входа приложения
├── internal/
│   ├── config/                 # Конфигурация приложения
│   ├── db/
│   │   ├── models/             # Модели базы данных, в том числе для GORM автомиграций
│   │   └── postgres.go         # Подключение к БД
│   ├── error/                  # Кастомные ошибки и валидация данных
│   ├── router/                 # Маршрутизатор HTTP, регистрация всех зависимостей(не успел
|   |                             подключить DI :( )
│   ├── pr/                     # Фича Pull Request
│   ├── team/                   # Фича команд
│   └── user/                   # Фича пользователей
├── docker-compose.yaml         
├── Dockerfile                  
└── .env                        # Переменные окружения
```

### Описание ключевых компонентов

**Компоненты:**
- `application/` - DTO, request/response, трансформеры
- `infrastructure/` - HTTP контроллеры и репозитории
- `usecase/` - бизнес-логика приложения

## ⚡ Нагрузочное тестирование при помощи K6

Нагрузочное тестирование проводилось с помощью **K6** для оценки производительности API endpoints.

### Конфигурация теста

``./load_testing/test.js``

```javascript
export const options = {
    stages: [
        {duration: '1m', target: 100},
        {duration: '1m', target: 100},
        {duration: '2m', target: 300},
        {duration: '1m', target: 150},
    ],
    thresholds: {
        http_req_failed: ['rate<0.01'],
        http_req_duration: ['p(95)<301'],
        checks: ['rate>0.95'],
    },
};
```

### Результаты тестирования
``./load_testing/test_result.txt``

```
  █ TOTAL RESULTS 

    checks_total.......: 182456  602.09739/s
    checks_succeeded...: 100.00% 182456 out of 182456
    checks_failed......: 0.00%   0 out of 182456

    ✓ status is 201
    ✓ response has team data
    ✓ team name matches
    ✓ response time acceptable
    ✓ has correct content type
    ✓ team structure is correct
    ✓ members have UUID v4 format
    ✓ members have required fields

    HTTP
    http_req_duration..............: avg=45.52ms min=17.27ms med=39.72ms max=412.88ms p(90)=68.92ms p(95)=88.24ms
      { expected_response:true }...: avg=45.52ms min=17.27ms med=39.72ms max=412.88ms p(90)=68.92ms p(95)=88.24ms
    http_req_failed................: 0.00% 0 out of 22807
    http_reqs......................: 22807 75.262174/s

    EXECUTION
    iteration_duration.............: avg=2.05s   min=1.01s   med=2.04s   max=3.33s    p(90)=3.04s   p(95)=3.06s  
    iterations.....................: 22807 75.262174/s
    vus............................: 2     min=2          max=300
    vus_max........................: 300   min=300        max=300

    NETWORK
    data_received..................: 13 MB 44 kB/s
    data_sent......................: 13 MB 43 kB/s




running (5m03.0s), 000/300 VUs, 22807 complete and 0 interrupted iterations
default ✓ [ 100% ] 000/300 VUs  5m0s
```

**Ключевые метрики:**
- ✅ **95-й процентиль** времени ответа: **88.24ms**
- ✅ Система стабильно работает под нагрузкой **300**
- ✅ Среднее время ответа: **45.52ms**